...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      // BUG CORRIGÉ (v7 → v7-fix) : `caches.has()` N'EXISTE PAS dans l'API
      // Cache Storage (aucun navigateur ne l'implémente). L'appeler faisait
      // planter activate() avec une TypeError avant même d'avoir pu copier
      // TEMP_CACHE_NAME vers CACHE_NAME et avant self.clients.claim() — donc
      // le nouveau Service Worker ne prenait JAMAIS le contrôle, quel que
      // soit le contenu réellement téléchargé par install(). C'est très
      // probablement la cause du "la v7 ne se met pas à jour" : install()
      // réussissait, mais activate() échouait silencieusement juste après.
      // Remplacé par caches.keys() (l'unique façon correcte de lister les
      // caches existants), en ne changeant rien d'autre à la logique.
      const existingCacheNames = await caches.keys();
      const tempExists = existingCacheNames.includes(TEMP_CACHE_NAME);
      if (tempExists) {
