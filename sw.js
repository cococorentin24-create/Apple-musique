...
const CRASH_LOG_MAX_ENTRIES = 20; // boîte noire courte : les derniers crashs suffisent, pas un historique illimité
const ASSETS = [
  './',
  './index.html'
];
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        allKeys
          .filter((k) => k !== CACHE_NAME && k !== CRASH_LOG_CACHE_NAME)
