...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    (async () => {
      ...
      // disparu du Cache Storage (cause encore non identifiée avec
      // certitude), ou
      ...
      try {
        ...
        // Dernier recours : au lieu de texte brut inerte (qui ne pouvait
        // JAMAIS se corriger tout seul — confirmé : le seul moyen de sortir
        // de cet écran était un nouveau fichier sw.js déployé, car ce même
        // fetch handler ne retélécharge jamais sw.js lui-même, seulement le
        // contenu), une vraie page HTML minimale avec un script qui :
        //   1. Force navigator.serviceWorker.update() — une VRAIE
        //      vérification réseau du fichier sw.js, indépendante du cache
        //      Storage cassé (update() compare toujours le fichier au
        //      réseau, ignore le Cache Storage applicatif).
        //   2. Si un nouveau SW est trouvé, le cycle install/activate normal
        //      reprend et recharge automatiquement une fois prêt.
        //   3. Sinon (fichier sw.js inchangé), retente un fetch réseau
        //      direct pour le contenu demandé, à intervalle croissant,
        //      avec un statut TOUJOURS visible — jamais un blocage muet.
...
(function() {
  ...
  var attempt = 0;
  var maxAttempts = 8; // ~2 minutes de tentatives avant d'abandonner visiblement
  var baseDelay = 2000;

  function setStatus(msg) { if (statusEl) statusEl.textContent = msg; }

  // Étape 1 : force une vraie vérification réseau de sw.js lui-même —
  // c'est ce qui manquait avant : le fetch handler cassé ne pouvait jamais
  // se réparer tout seul car il ne retélécharge jamais sw.js, seulement le
  // contenu. update() ignore le Cache Storage applicatif et compare
  // toujours le fichier sw.js au réseau.
  function tryServiceWorkerUpdate() {
    var handled = false; // empêche tryDirectFetch() d'être appelée 2 fois
                          // depuis 2 branches différentes de cette même
                          // chaîne .then() — bug réel trouvé en testant :
                          // sans ce flag, la branche "!reg" appelait
                          // tryDirectFetch() puis retournait undefined,
                          // que le .then() SUIVANT recevait comme valeur
                          // résolue et redéclenchait tryDirectFetch() une
                          // 2e fois (2 fetch, 2 reload constatés en test
                          // automatisé avant cette correction).
    function callOnce() {
      if (handled) return;
      handled = true;
      tryDirectFetch();
    ...
    navigator.serviceWorker.getRegistration().then(function(reg) {
      ...
      // Si un nouveau SW prend le contrôle suite à cet update(), un reload
      // automatique s'ensuit — pas besoin de gérer ce cas ici, le SW
      // (controllerchange déjà géré dans index.html) s'en charge une fois
      // que index.html aura effectivement rechargé et pourra tourner.
      reg.addEventListener('updatefound', function() {
        setStatus('Mise à jour trouvée, installation…');
      });
      return reg.update();
    }).then(function() {
      // N'appelle callOnce() que si aucune branche précédente ne l'a déjà
      // fait (cas normal : reg existait, update() a fini sans détecter de
      // nouveau SW — fichier inchangé, on retente directement le contenu).
      callOnce();
    }).catch(function() {
      callOnce();
    });
  ...
  // Étape 2 : retry direct du contenu (contourne le SW pour cette tentative
  // précise — une requête envoyée pendant que la page est déjà chargée
  // n'est plus interceptée par le même chemin de fetch handler cassé de la
  // même façon qu'une navigation initiale, et si le cache s'est entre-temps
  // réparé — ex: un activate() a eu le temps de finir en tâche de fond —
  // ce fetch peut réussir même sans changement de sw.js).
  ...
  function scheduleRetry() {
    var delay = Math.min(baseDelay * attempt, 20000); // croissant, plafonné à 20s
  ...
  tryServiceWorkerUpdate();
        ...
        // Dernier recours absolu : une vraie Response, jamais null/undefined
        // (règle d'or documentée en tête du fetch handler), avec du HTML
        // exécutable au lieu de texte brut inerte.
