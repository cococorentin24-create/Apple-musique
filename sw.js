...
const APP_VERSION = 'v3'; // ← CHANGE CE NUMÉRO À CHAQUE NOUVELLE VERSION D'INDEX.HTML
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        // cache.addAll() est atomique : si UN SEUL fichier échoue à se
        // télécharger, toute l'opération échoue et RIEN n'est écrit dans
        // tempCache — impossible d'avoir un mélange de fichiers anciens et
        // nouveaux avec cette méthode.
        await tempCache.addAll(ASSETS);
...
self.addEventListener('fetch', (event) => {
  ...
  event.respondWith(
    caches.match(event.request).then((cached) => {
      ...
      // etc.) remonterait comme une erreur non interceptée dans
      // respondWith() au lieu de simplement échouer proprement.
