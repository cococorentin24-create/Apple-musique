...
//
//   CONFIRMÉ EN USAGE RÉEL (v6 → v7) : plusieurs correctifs livrés dans
//   index.html (covers manquantes jamais retrouvées côté sync) sont restés
//   invisibles sur un appareil malgré le fichier HTML correctement remplacé
//   sur l'hébergement, PARCE QUE ce fichier sw.js, lui, n'avait pas changé
//   d'un seul octet. Le navigateur a donc continué de considérer qu'aucune
//   mise à jour n'existait, et a continué à servir l'ancien index.html
//   depuis CACHE_NAME='coco-music-v6' — indépendamment du bouton de reset
//   manuel ou de la vérification automatique (voir index.html), qui
//   dépendent tous deux de ce même mécanisme natif pour avoir quoi que ce
//   soit de nouveau à détecter en premier lieu. Rappel donc, à chaque
//   livraison d'index.html : incrémenter ce numéro, sans exception, même
//   pour un changement qui semble mineur.
const APP_VERSION = 'v8-diag'; // ← CHANGE CE NUMÉRO À CHAQUE NOUVELLE VERSION D'INDEX.HTML
  ...
  event.respondWith(
    (async () => {
      ...
      try {
        ...
        // DIAGNOSTIC TEMPORAIRE (pour identifier avec CERTITUDE la cause du
        // message générique, plutôt que deviner) : avant de renvoyer le
        // message de dernier recours, on liste ce qui existe RÉELLEMENT en
        // cache à cet instant précis, et on l'affiche à l'écran au lieu du
        // texte fixe. Zéro changement de comportement (toujours une vraie
        // Response 503, jamais null/undefined) — seul le contenu affiché
        // change, temporairement, le temps de voir exactement ce qui
        // manque. Sûr à garder même en usage normal (n'affecte jamais un
        // scénario qui fonctionnait déjà, puisqu'on n'atteint cette ligne
        // que dans le cas déjà cassé).
        let debugInfo = '(diagnostic indisponible)';
        try {
          const allCacheNames = await caches.keys();
          const perCache = await Promise.all(allCacheNames.map(async (name) => {
            const c = await caches.open(name);
            const reqs = await c.keys();
            return name + ': [' + reqs.map(r => new URL(r.url).pathname).join(', ') + ']';
          }));
          debugInfo = 'URL demandée: ' + url.pathname + '\n' +
            'mode: ' + event.request.mode + '\n' +
            'wantsHTML: ' + wantsHTML + '\n' +
            'Caches trouvés: ' + (allCacheNames.length ? '' : '(aucun)') + '\n' +
            perCache.join('\n');
        } catch (diagErr) { debugInfo = 'erreur diagnostic: ' + diagErr.message; }
