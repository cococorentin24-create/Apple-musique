...
// ---------------------------------------------------------------------------
// CONTENT UPDATE CHECK — logique partagée par le handler CHECK_CONTENT_UPDATE
// ci-dessous. Même raisonnement de verrou que _selfRepairCache juste au-
// dessus : plusieurs onglets/fenêtres de la même PWA ouverts en même temps
// (ou plusieurs appels rapprochés depuis un seul onglet) partagent tous le
// MÊME Service Worker actif — sans ce verrou, deux appels concurrents
// pourraient chacun lire _getActiveCacheName() avant que l'autre n'ait fini
// de le mettre à jour, et l'un supprimerait alors (caches.delete) le cache
// que l'autre vient tout juste d'activer. Un seul téléchargement/bascule
// réel a lieu à la fois ; les appelants concurrents reçoivent tous le même
// résultat une fois qu'il aboutit — aucun appelant n'est laissé sans
// réponse.
// ---------------------------------------------------------------------------
let _contentUpdateInFlight = null;
async function _checkContentUpdate() {
  if (_contentUpdateInFlight) return _contentUpdateInFlight;
  _contentUpdateInFlight = (async () => {
    try {
      const htmlReq = new Request('./index.html', { cache: 'reload' });
      const htmlRes = await fetch(htmlReq);
      ...
      return { updated: false, error: String(err && err.message || err) };
    } finally {
      _contentUpdateInFlight = null;
    }
  })();
  return _contentUpdateInFlight;
}
  ...
  if (event.data && event.data.type === 'RESET_APP_CACHE') {
    ...
    event.waitUntil(
      (async () => {
        const client = event.source;
        try {
          const result = await _checkContentUpdate();
          if (client) client.postMessage(Object.assign({ type: 'CONTENT_UPDATE_RESULT' }, result));
        } catch (err) {
